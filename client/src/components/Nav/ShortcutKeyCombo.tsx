...
function ShortcutKbd({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <kbd
      className={cn(
        'border-border-light bg-surface-primary-alt text-text-primary inline-flex h-[22px] min-w-[22px] items-center justify-center rounded-md border px-1.5 font-sans text-[11px] leading-none font-medium',
        className,
      )}
    >
      {children}
