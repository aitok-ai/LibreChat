      ...
      >
        ...
        {!isEditing && (
          <button
            ...
            className="focus-visible:ring-ring-primary absolute inset-0 z-10 rounded-xl focus:outline-none focus-visible:ring-2"
          ...
          render={({ field }) =>
            isEditing ? (
              <TextareaAutosize
                ...
                autoFocus
                className="text-text-primary placeholder:text-text-secondary focus-visible:ring-ring-primary w-full resize-none overflow-y-auto bg-transparent font-mono text-sm leading-relaxed focus:outline-none focus-visible:ring-2 sm:text-base"
                minRows={4}
                maxRows={16}
                onKeyDown={(e) => {
                  if (e.key === 'Escape') {
