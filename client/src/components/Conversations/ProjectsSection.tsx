      ...
      <OGDialogContent className="w-11/12 max-w-md" showCloseButton={false}>
        ...
        <form id={formId} onSubmit={handleSubmit}>
          <Input
            ...
            className="text-text-primary placeholder:text-text-secondary focus-visible:ring-ring-primary w-full bg-transparent focus-visible:ring-2"
          />
        </form>
  ...
  function ProjectItem({ project, toggleNav, defaultExpanded }: ProjectItemProps) {
    ...
    return (
      <li className="list-none">
        <div className="group/project-row text-text-primary hover:bg-surface-active-alt relative flex h-9 items-center rounded-lg text-sm transition-colors">
          <button
            type="button"
          ...
          >
            <ChevronRight
              className={cn(
                'text-text-secondary h-3.5 w-3.5 shrink-0 transition-transform duration-200',
                expanded && 'rotate-90',
              )}
              aria-hidden="true"
            />
          ...
          <div className="bg-surface-active-alt absolute top-1/2 right-2 flex -translate-y-1/2 items-center gap-0.5 rounded-md opacity-0 transition-opacity group-focus-within/project-row:opacity-100 group-hover/project-row:opacity-100 has-[[data-state=open]]:opacity-100">
            <TooltipAnchor
...
const ProjectsSection = ({ toggleNav, isAuthenticated }: ProjectsSectionProps) => {
  ...
  return (
    <div className="flex flex-col px-3 text-sm">
      ...
      {isExpanded && (
        <div className="max-h-[42vh] scrollbar-gutter-stable overflow-y-auto">
          {renderProjectsBody()}
        </div>
      )}
