...
function ProjectConversationDialog({
  ...
  return (
    ...
    >
      ...
      <label className="text-text-primary flex flex-col gap-2 text-sm">
        {localize('com_ui_select_project')}
        <select
          value={selectedProjectId}
          onChange={(event) => setSelectedProjectId(event.target.value)}
          className="border-border-light bg-surface-primary focus:ring-ring-primary h-10 rounded-lg border px-3 text-sm outline-none focus:ring-2"
        >
      ...
      {hasNextPage && (
        <button
          type="button"
          className="text-text-secondary hover:text-text-primary mt-3 text-sm"
