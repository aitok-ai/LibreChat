...
function ActivityPart({
    ...
    return (
      <Container>
        ...
        {item.textTruncated === true && (
          <div className="text-text-secondary mt-2 text-xs italic">
  ...
  if (item.type === 'reasoning') {
    if (item.text == null || item.text === '') {
      return (
        <div className="text-text-secondary my-2 text-sm" role="status">
  ...
  const truncationNotice =
    item.inputTruncated === true || item.outputTruncated === true ? (
      <div className="text-text-secondary mb-2 text-xs italic">
        {localize('com_ui_subagent_thread_message_truncated')}
      </div>
    ) : null;
    ...
    >
      <div className="border-border-light flex min-h-[2.75rem] items-center justify-between gap-3 border-b px-3 py-2">
        ...
        <Button
          ...
          title={toggleLabel}
          className="text-text-secondary hover:bg-surface-tertiary hover:text-text-primary focus:ring-text-primary h-8 gap-1.5 rounded-md px-2 text-xs font-medium transition focus:ring-2 focus:outline-none"
        >
...
export default function SubagentActivity({
  ...
  if (state === 'loading') {
    body = (
      <div className="text-text-secondary py-8 text-center text-sm" role="status">
    ...
    body = (
      <div className="border-border-light bg-surface-secondary text-text-secondary rounded-lg border p-3 text-sm">
        {isSubmitting
      ...
      >
        {!isAtBottom && (
          <Button
            ...
            className="border-border-light bg-surface-secondary text-text-secondary sticky top-[calc(100%-2.75rem)] z-10 ml-auto h-8 w-8 rounded-full border shadow-md"
          >
