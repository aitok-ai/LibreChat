...
function TriggerIcon({ kind }: { kind: ChildConversationTurn['trigger']['kind'] }) {
  ...
  return (
    <span className="bg-surface-tertiary text-text-secondary flex size-6 items-center justify-center rounded-full">
...
function ExternalEventTrigger({ turn }: { turn: ChildConversationTurn }) {
  ...
  if (details == null) {
    return (
      <div className="text-text-secondary flex min-h-9 items-center gap-2 text-sm">
    ...
    <Collapsible open={expanded} onOpenChange={setExpanded}>
      <CollapsibleTrigger asChild>
        <Button
          type="button"
          variant="ghost"
          className="text-text-secondary hover:text-text-primary h-auto min-h-9 w-full justify-start gap-2 px-0 text-left text-sm hover:bg-transparent"
        >
          <TriggerIcon kind="external_event" />
          <span className="font-medium">{label}</span>
      ...
      <CollapsibleContent className="border-border-light text-text-secondary ml-8 border-l py-1 pl-3 text-xs">
...
function TriggerMessage({ turn, fullWidth }: { turn: ChildConversationTurn; fullWidth: boolean }) {
  ...
  return (
    <MessageRow
      ...
      fullWidth={fullWidth}
    >
      <div className="text-text-secondary mb-1 flex items-center gap-1.5 text-xs font-medium">
