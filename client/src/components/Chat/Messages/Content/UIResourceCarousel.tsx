    ...
    >
      ...
      {showLeftArrow && (
        <button
          type="button"
          onClick={() => scroll('left')}
          className={`absolute top-1/2 left-2 z-20 -translate-y-1/2 rounded-xl bg-white p-2 text-gray-800 shadow-lg transition-all duration-200 hover:scale-110 hover:bg-gray-100 hover:shadow-xl active:scale-95 dark:bg-gray-200 dark:text-gray-800 dark:hover:bg-gray-300 ${
      ...
      >
        {uiResources.map((uiResource, index) => {
          ...
          return (
            <div
              key={index}
              className="animate-in fade-in-0 slide-in-from-bottom-5 flex-shrink-0 transform-gpu transition-all duration-300 ease-out"
              style={{
                width: `${width}px`,
                minHeight: `${height}px`,
      ...
      {showRightArrow && (
        <button
          ...
          className={`absolute top-1/2 right-2 z-20 -translate-y-1/2 rounded-xl bg-white p-2 text-gray-800 shadow-lg transition-all duration-200 hover:scale-110 hover:bg-gray-100 hover:shadow-xl active:scale-95 dark:bg-gray-200 dark:text-gray-800 dark:hover:bg-gray-300 ${
            isContainerHovered ? 'opacity-100' : 'pointer-events-none opacity-0'
          }`}
