...
export default function AskUserQuestionCall({
  ...
  if (batch != null) {
    ...
    return (
      <>
        <div className="border-border-light bg-surface-secondary my-2 flex w-full flex-col rounded-lg border">
          <div className="text-text-secondary flex items-center gap-2 px-3 py-2 text-xs font-medium">
            {failed ? (
              <TriangleAlert className="text-text-warning h-4 w-4" aria-hidden="true" />
            ) : (
          ...
          <div className="px-3 pb-3">
            {batch.questions.map((item, index) => {
              ...
              return (
                <div key={item.id} className={index > 0 ? 'border-border-light border-t pt-3' : ''}>
                  {item.header != null && (
                    <p className="text-text-secondary text-xs font-medium">{item.header}</p>
                  )}
                  ...
                  {typeof answer === 'string' && (
                    <p className="text-text-primary mt-1 text-sm [overflow-wrap:anywhere]">
  ...
  if (failed) {
    return (
      <>
        <div
          className="border-border-light bg-surface-secondary my-2 flex w-full flex-col gap-1.5 rounded-lg border p-3"
          role="status"
        >
          <div className="text-text-warning flex items-center gap-2 text-xs font-medium">
    ...
    <>
      <div className="border-border-light bg-surface-secondary my-2 flex w-full flex-col gap-1.5 rounded-lg border p-3">
        ...
        {question?.description != null && question.description.length > 0 && (
          <p className="text-text-secondary text-sm [overflow-wrap:anywhere]">
            {question.description}
          </p>
        )}
        {answered ? (
          <p className="text-text-primary text-sm [overflow-wrap:anywhere]">
            <span className="text-text-secondary font-medium">
              {localize('com_ui_you_answered')}
            </span>{' '}
            {answerLabel}
