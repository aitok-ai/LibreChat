      ...
      {active === 'edit' && (
        <div className="flex flex-col gap-1">
          <TextareaAutosize
            ...
            maxRows={16}
            className={cn(
              'bg-surface-primary w-full resize-none rounded-md border p-2 font-mono text-xs',
              editIsValid ? 'border-border-light' : 'border-red-500',
            )}
            aria-label={localize('com_ui_edit')}
          />
          {!editIsValid && (
            <span className="text-text-warning text-xs">{localize('com_ui_invalid_json')}</span>
          )}
        </div>
      )}
