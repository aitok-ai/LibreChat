  ...
  return (
    <>
      <button
        ...
        className={cn(
          'group border-border-light bg-surface-secondary hover:bg-surface-tertiary my-1.5 flex w-full flex-col gap-1 rounded-lg border px-3 py-2 text-left transition',
        ...
        <div className="text-text-primary flex items-center gap-2 text-sm font-medium">
          <div
            className={cn(
              ...
              running && !subagentAgent && 'text-primary animate-pulse',
            )}
            aria-hidden="true"
          >
            {subagentAgent ? (
      ...
      <OGDialog open={open} onOpenChange={setOpen}>
        ...
        >
          ...
          <div className="border-border-light bg-surface-primary relative min-h-0 flex-1 border-t">
            {!isAtBottom && (
              <button
                type="button"
                onClick={scrollDialogToBottom}
                aria-label={localize('com_ui_subagent_scroll_to_bottom')}
                className="border-border-light bg-surface-secondary text-text-secondary hover:bg-surface-tertiary hover:text-text-primary absolute right-4 bottom-3 z-10 flex h-8 w-8 items-center justify-center rounded-full border shadow-md transition"
              >
    ...
    >
      <div className="border-border-light flex min-h-[2.75rem] items-center justify-between gap-3 border-b px-3 py-2">
        ...
        <button
          ...
          title={toggleLabel}
          className="text-text-secondary hover:bg-surface-tertiary hover:text-text-primary focus:ring-ring inline-flex h-8 items-center gap-1.5 rounded-md px-2 text-xs font-medium transition focus:ring-2 focus:outline-none"
        >
          {expanded ? (
      ...
      >
        <div className="markdown prose prose-sm message-content light dark:prose-invert text-text-primary w-full max-w-none break-words dark:text-gray-100">
