  ...
  return (
    <div className="group flex items-center justify-between gap-4 text-sm">
      ...
      <span className="text-text-primary font-medium">
        {formatTokens(value)}
        {percent != null && (
          <span className={`text-text-secondary ml-1 text-xs ${HOVER_INK}`} aria-hidden="true">
            ({Math.round(percent)}%)
          </span>
        )}
      </span>
  ...
  return (
    <div className="w-72" role="region" aria-label={localize('com_ui_context_usage')}>
      <Collapsible open={expanded} onOpenChange={setExpanded} className="space-y-3">
        <CollapsibleTrigger
          className="group focus-visible:ring-ring-primary flex w-full items-center justify-between gap-2 rounded-sm focus-visible:ring-2 focus-visible:outline-none"
        ...
        >
          ...
          <span className="text-text-secondary flex items-center gap-1 text-xs font-medium whitespace-nowrap">
            ...
            <ChevronDown
              aria-hidden="true"
              className="text-text-tertiary size-3.5 shrink-0 transition-transform duration-200 group-data-[state=open]:rotate-180 motion-reduce:transition-none"
            />
          </span>
        </CollapsibleTrigger>
        ...
        <CollapsibleContent className="data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down space-y-3 overflow-hidden motion-reduce:animate-none">
          <div
