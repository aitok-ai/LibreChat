    ...
    >
      {skills.map((name) => (
        <span
          key={name}
          role="listitem"
          className="border-border-light bg-surface-secondary text-text-secondary inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs"
        >
          ...
          <button
            ...
            onClick={() => remove(name)}
            className="text-text-secondary hover:bg-surface-tertiary hover:text-text-primary -mr-0.5 ml-0.5 rounded-full p-0.5"
          >
