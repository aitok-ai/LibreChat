    ...
    return (
      <Ariakit.HovercardProvider placement="top-end" showTimeout={100} hideTimeout={150}>
        <Ariakit.HovercardAnchor
          render={
            <button
              ...
              className={cn(
                'size-theme-control rounded-theme-control-round bg-text-primary p-theme-compact text-text-primary duration-theme-normal disabled:text-text-secondary outline-offset-4 transition-all disabled:cursor-not-allowed disabled:opacity-10',
              )}
        ...
        <Ariakit.Hovercard
          ...
          className="border-border-light bg-surface-secondary text-text-primary z-50 min-w-[12rem] rounded-xl border p-1.5 shadow-lg outline-none"
        >
          {rows.map((row) => (
            <button
              key={row.key}
