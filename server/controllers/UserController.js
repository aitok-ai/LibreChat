...
} = require("../middleware/messageQuota");
...
const getUserController = async (req, res) => {
  try {
    ...
    if (userId === undefined || userId === req.user.id) {
      ...
      const monthlyQuotaConsumed = await getUserMessageQuotaUsagePastDays(
        req.user,
        30
      ); // This value might be dynamic based on your application logic
      ...
      res
        .status(200)
        .send({ id, name, username, followers, following, biography });
    }
  } catch (error) {
    console.log(error);
...
const postBiographyController = async (req, res) => {
  try {
    ...
    if (userId !== req.user.id) {
      return res
        .status(403)
        .json({ message: "Unauthorized to update biography" });
    }
...
const usernameController = async (req, res) => {
  try {
    ...
    if (userId !== req.user.id) {
      return res
        .status(403)
        .json({ message: "Unauthorized to update username" });
    }
...
const followUserController = async (req, res) => {
  try {
    ...
    otherUserUpdate[`followers.${user.id}`] = {
      name: user.name,
