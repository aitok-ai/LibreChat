...
router.get("/", async (req, res) => {
  ...
  if (!req.user) {
    ...
    res
      .status(200)
      .send(
        await getConvosByPage(req.user.id, pageNumber, pageSize, isArchived)
      );
  }
});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const job = await jobScheduler.now(
        IMPORT_CONVERSATION_JOB_NAME,
        filepath,
        req.user.id
      );
...
router.post("/:conversationId/viewcount/increment", async (req, res) => {
  const { conversationId } = req.params;
  console.log(
    `routes: hit viewcount increment router for conversationId ${conversationId}`
  );
  try {
