...
export default function Badge({
  ...
  const handleRootClick: React.MouseEventHandler<HTMLElement> = customOnClick
    ? (event) => customOnClick(event as React.MouseEvent<HTMLButtonElement>)
    : handleClick;
  ...
  const badgeContent = (
    <>
      ...
      {isEditing && !isDragging && (
        <motion.button
          type="button"
          className="@container-[600px]:h-5 @container-[600px]:w-5 bg-surface-secondary-alt text-text-primary absolute -top-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full shadow-sm"
