...
const Radio: React.NamedExoticComponent<RadioProps> = memo(function Radio({
  ...
  return (
    <div
      className={`relative ${fullWidth ? 'flex' : 'inline-flex'} bg-muted items-center rounded-lg ${className}`}
      role="radiogroup"
      aria-labelledby={ariaLabelledBy}
    >
      {selectedIndex >= 0 && isMounted && (
        <div
          className="border-border/50 bg-background pointer-events-none absolute inset-y-0 rounded-md border shadow-sm transition-all duration-300 ease-out"
