...
const Radio: React.NamedExoticComponent<RadioProps> = memo(function Radio({
  ...
  return (
    <div
      ref={containerRef}
      className={`relative ${fullWidth ? 'flex' : 'inline-flex'} bg-surface-tertiary items-center rounded-lg px-1 ${className}`}
      role="radiogroup"
      aria-labelledby={ariaLabelledBy}
    >
      {selectedIndex >= 0 && isMounted && (
        <div
          className="border-border-light bg-surface-primary pointer-events-none absolute inset-y-1 left-0 rounded-md border shadow-sm transition-all duration-300 ease-out"
      ...
      {options.map((option, index) => (
        <button
          ...
          disabled={disabled}
          className={`focus-visible:ring-text-primary relative z-10 flex h-[34px] items-center justify-center gap-2 rounded-md px-4 text-sm font-medium transition-colors duration-150 focus-visible:ring-2 focus-visible:outline-none ${
