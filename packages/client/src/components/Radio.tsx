...
const Radio: React.NamedExoticComponent<RadioProps> = memo(function Radio({
  ...
  return (
    <div
      ref={containerRef}
      className={`relative ${fullWidth ? 'flex' : 'inline-flex'} bg-muted items-center rounded-lg px-1 ${className}`}
      role="radiogroup"
      aria-labelledby={ariaLabelledBy}
    >
      {selectedIndex >= 0 && isMounted && (
        <div
          className="border-border bg-background pointer-events-none absolute inset-y-1 left-0 rounded-md border shadow-sm transition-all duration-300 ease-out"
